...
public class CallImpl {
     ...
     * 通过id查询
     */
    public static Call selectBySon(String sno) {
        connection = MysqlJDBC.getConnection();
        String sql = "SELECT * FROM t_call_car_record where id = ?";
        Call call = null;
        try {
            ...
            if (resultSet.next()) {
                ...
                call.setResponsible(resultSet.getString("responsible"));
                call.setCar_id(resultSet.getInt("car_id"));
                call.setDate_time(resultSet.getString("date_time"));
                call.setIs_return(resultSet.getString("is_return"));
        ...
        return call;//返回departmentList对象
    }

    /**
     * 查询全部部门
     */
    public static List<Call> selectAll() {
        connection = MysqlJDBC.getConnection();
        String sql = "SELECT * FROM t_call_car_record";
        List<Call> callArrayList = new ArrayList<>();
        try {
            statement = connection.prepareStatement(sql);
            resultSet = statement.executeQuery();
            //处理查询到的数据
